...
from consumer.helper.authorization import verify_password, get_password_hash
...
class VerifyResult(BaseModel):
    verify: bool
    message: str
async def authorization_create(key: str, db: Session = Depends(get_db)) -> VerifyResult:

    check_main_key = await authorization_main(key, db)
...
async def authorization_update(key: str, db: Session = Depends(get_db)) -> VerifyResult:
...
async def authorization_delete(key: str, db: Session = Depends(get_db)) -> VerifyResult:
...
async def authorization_main(key: str, db: Session = Depends(get_db)) -> VerifyResult:
